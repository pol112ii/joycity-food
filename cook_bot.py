...
# "위치"만 보면 늘 늦게 반응해 목표를 지나치고 반대로 튕기며 진동함.
# → 지금 이동 "속도"로 앞을 예측해서, 목표를 지나칠 것 같으면 미리 반대 버튼을 톡 눌러 브레이크.
LOOKAHEAD    = 1.1      # 속도로 이 초만큼 앞을 예측 (크게=더 일찍 브레이크)
CTRL_DEADBAND = 0.04    # 예측 위치가 목표 중심에서 이 안이면 그냥 지켜봄
TAP_MIN, TAP_MAX = 0.08, 0.18   # 버튼을 톡 누르는 시간(초)
TAP_COOLDOWN = 0.4      # 한 번 누른 뒤 이만큼은 지켜보기만
STALL_V      = 0.04     # 이 속도(/초)보다 느리면 '멈춘 것'으로 보고 필요 시 재차 톡
...
def _worker_loop():
    """속도 예측 제어: 이동 속도로 앞을 예측해 목표를 지나칠 것 같으면
    미리 반대 버튼을 톡 눌러 브레이크. 방향 바꿀 때만 한 번씩 톡 누름."""
    global running, alive, need_start
    history = []       # (시각, 위치) 이력 → 속도 계산
    cur_dir = None
    last_tap = 0.0
    with mss.mss() as sct:
        while alive:
